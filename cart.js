		...
		var arr = [];
			...
			/*else{*/
				...
				$('.modal-body').append("<div class='item"+ i + "'> </div>");
					id=i;
				$('.item'+i).append("<table class='table' style='width: 100%'> <tr> <td align='center' style='width: 200px'>"+ proizvod.naziv + "</td> <td align='center'>" + proizvod.kolicina+ "</td> <td align='center'>"+proizvod.cena*proizvod.kolicina+
					"</td><td align='center'><button  name='btn' class='btn btn-danger' value='"+i+"'>Ponisti</button></td></tr></table>" );
				}//<--
			...
			isempty=0;
/*}*/
			$('.ukupno').append('<h4 align="center"> Ukupno za placanje: ' + suma + ',00 din</h4>');
			...
			$('.btn-danger').on('click', function(){
					...
					$('.ukupno').find('h4').text('Ukupno za placanje: ' + suma + ',00 din');
...
$(document).click(function(e) {
    ...
    $("#naruci").on('click',function(){
    	...
    	if(suma==0){alert('Morate izabrati bar jedan proizvod za dostavu!'); }else{
    	$('#myModal').modal('hide');
    	$('#forma').modal('show');
    	...
    	if($('#ime').val()!=''){
    		if($('#prezime').val()!=''){		
    				if($('#email').val()!=''){
    					if($('#adresa').val()!=''){
    						if($('#telefon').val()!=''){
    							alert('Uspesno ste porucili hranu!');
    							localStorage.clear();
    							$('#forma').modal('hide');
